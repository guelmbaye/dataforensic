...
MCP_TOOL_CANDIDATES: dict[str, list[str]] = {
    "get_asset_context": ["get_dataset", "get_entity", "get_asset", "describe_entity"],
    "get_schema": ["get_dataset_schema", "get_schema", "get_dataset"],
    "get_lineage": ["get_lineage", "traverse_lineage", "get_dataset_lineage"],
    "get_ownership": ["get_ownership", "get_dataset", "get_entity"],
    "get_quality_context": ["get_assertions", "get_data_quality", "get_dataset_assertions"],
    "find_changes": ["get_timeline", "get_schema_history", "get_changes"],
    "search_assets": ["search", "search_entities", "find_entities"],
}
...
class LiveDataHubProvider(DataHubProvider):
    ...
    async def find_changes(
        ...
        for transaction in transactions:
            ...
            moment = (
                to_iso(datetime.fromtimestamp(timestamp / 1000))
                if isinstance(timestamp, (int, float))
                else timestamp
            )
            for change in transaction.get("changeEvents", []) or []:
                changes.append(
                    {
                        "timestamp": moment,
                        "entity_urn": urn,
                        "type": "SCHEMA_CHANGE"
                        if change.get("category") == "TECHNICAL_SCHEMA"
                        else str(change.get("category")),
                        "operation": change.get("operation"),
                        "summary": change.get("description")
                        or f"{change.get('operation')} {change.get('modifier')}",
                        "details": {
                            "field": change.get("modifier"),
    ...
    async def write_incident_memory(
        ...
        try:
            ...
            return self._fail("write_incident_memory", f"addLink failed: {exc}")
        if "addLink" not in operations:
            return self._fail("write_incident_memory", "No write operation succeeded")
        return self._ok(
